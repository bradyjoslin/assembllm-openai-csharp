...
public class Program
{
    ...
    public static int Completion()
    {
        ...
        var (completionResponse, bad_request) = GetCompletionsResponse(completionRequest);
        if (bad_request is not null)
        {
            Pdk.Log(LogLevel.Error, $"Error getting response: {bad_request.Message}");
            Pdk.SetError(bad_request.Message);
            return 1;
        }
    ...
    public static (CompletionsResponse, Exception) GetCompletionsResponse(CompletionRequest cReq)
    {
        ...
        if (response.Status != 200)
        {
            ...
            return (new CompletionsResponse(), new FormatException($"error calling API: {response.Status} {responseBody}"));
        }
